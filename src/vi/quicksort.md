...
Dùng [Knuth shuffle](https://en.wikipedia.org/wiki/Fisher-Yates_shuffle) với độ phức tạp $\mathcal{O}(N)$ ta có thể đảm bảo hiệu quả của thuật toán. Ngoài ra, việc xáo trộn mảng input khiến cải tiến này, thường được gọi với cái tên randomized quicksort, trở thành ví dự kinh điển trong việc phân tích các thuật toán ngẫu nhiên.
Mình có một [bài viết](http://dkhoa.me/post/randalgs_ex/) đề cập tới phân tích randomized quicksort.

>[!danger]
> Link không truy cập được.
...
Đây là thuật toán nổi tiếng do Dijkstra đề xuất cùng với [Dutch flag problem](https://en.wikipedia.org/wiki/Dutch_national_flag_problem).
Dùng để tránh trường hợp có quá nhiều giá trị trùng nhau trong dãy.
Điểm thú vị là phiên bản quicksort này được chính mình là tối ưu về mặt entropy.
Một chứng minh sơ lược có được đề cập trong Algorithms, R. Sedgewick.
